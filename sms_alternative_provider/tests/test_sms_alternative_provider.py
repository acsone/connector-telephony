...
class TestSmsAlternativeProvider(TransactionCase):
    ...
    def test_restrictions(self):
        ...
        # Test fallback behavior when no specific gateway matches
        # Set specific prefixes that don't match +1
        gw_no_restriction.prefix = "+2"  # Now only matches +2 numbers
        self.iap_gateway.prefix = "+3"  # Now only matches +3 numbers
        gw_32_49.prefix = "+49"  # Now only matches +49 numbers
        ...
        # Since all gateways now have prefix restrictions that don't match +1,
        # the SMS should still get a gateway assigned via fallback logic
        # (it should use a gateway with no prefix restriction, but we've set them all)

        # Let's check what happens - the SMS should still be processed
        with patch.object(type(self.env["sms.sms"]), "_send_with_api") as mock_send:
            ...
            # With the improved fallback logic, it should find SOME gateway
            # Even if no specific match, it should use a fallback
            self.assertEqual(sms.state, "sent")
            # The SMS should have SOME gateway assigned
            self.assertTrue(
                sms.sms_gateway_id,
                f"SMS should have a gateway assigned. "
                f"Current state: {sms.state}, gateway: {sms.sms_gateway_id}",
            )
