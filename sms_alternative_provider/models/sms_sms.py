...
class SmsSms(models.Model):
    ...
    def _split_by_api(self):
        ...
        for gateway_id, gateway_sms in groupby(
            ...
            if gateway_id and gateway_id.gateway_type != "iap":
                # For custom gateways, use None as API - they'll be handled in _send
                yield None, self.env["sms.sms"].concat(*gateway_sms_list)
            else:
        ...
        for sms in sms_without_gateway:
            ...
            for provider in providers:
                if provider._can_send({"number": sms.number}):
                    if not best_gateway or provider.sequence < best_gateway.sequence:
                        best_gateway = provider
                        _logger.info(
            ...
            if best_gateway:
                ...
                _logger.warning(
                    "No suitable gateway found for SMS %s with number %s",
                    sms.id,
                    sms.number,
                )
    ...
    def _send_with_custom_gateway(
        ...
        for gateway_id, sms_ids in gateway_groups.items():
            if not gateway_id:
                _logger.warning("Skipping SMS with no gateway: %s", sms_ids)
                continue
