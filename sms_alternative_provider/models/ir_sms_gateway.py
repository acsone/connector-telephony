...
class IrSmsGateway(models.Model):
    ...
    def _send(self, messages, handle_results=True, raise_exception=True):
        ...
        for provider, messages_to_send in provider2messages.items():
            ...
            # Get SMS records for these messages
            sms_records = SmsSms.browse([msg["id"] for msg in messages_to_send])

            # Update SMS records with gateway BEFORE sending
            sms_records.write({"sms_gateway_id": provider.id})

            provider_result = provider._send_via_self(sms_records) or []
    ...
    def _send_iap(self, messages):
        ...
        sms_api = sms_api_class(self.env.with_context(force_iap=True))
