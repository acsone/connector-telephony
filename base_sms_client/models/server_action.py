...
class ServerAction(models.Model):
    ...
    def run_action_sms(self, action, eval_context=None):
        """Run the SMS action for a single record."""
    ...
    def run_action_sms_multi(self, action, eval_context=None):
        """Run the SMS action for multiple records."""
        ...
        if self._context.get("active_id"):
            ...
            res_ids = (
                self.env[action.model_id.model].search([("mobile", "!=", False)]).ids
            )
